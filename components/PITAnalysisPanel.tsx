...
function StaffLoadTable({ staffAnalysis }: { staffAnalysis: StaffPITLoad[] }) {
    ...
    const relevant = staffAnalysis.filter(s => s.hoursUsed > 0 || s.pitContribution > 0);
...
export default function PITAnalysisPanel({ data, loading = false, onRegenerateWithPIT }: PITAnalysisPanelProps) {
    ...
    const metrics = useMemo(() => {
        if (!data) return null;
        const highPIT = data.highIntensitySlots.length;
        const violations = data.violations.length;
        const seniorPct = Math.round(data.seniorUtilization * 100);
        const maxPIT = Math.max(...(data.slots.map(s => s.pit) ?? [0]));
        return { highPIT, violations, seniorPct, maxPIT };
    }, [data]);
            ...
            <div className="divide-y divide-gray-100">
                ...
                {expandedSection === 'heatmap' && (
                    <div className="px-3 py-3">
                        <PITHeatmap slots={data.slots} />
                    </div>
                )}

                {/* Violations */}
                <SectionHeader
                    title={`Violazioni Presidio Senior (${data.violations.length})`}
                    icon={<AlertTriangle size={14} className={data.violations.length > 0 ? 'text-red-500' : 'text-green-500'} />}
                ...
                {expandedSection === 'staff' && (
                    <div className="px-3 py-3">
                        <StaffLoadTable staffAnalysis={data.staffAnalysis} />
                    </div>
                )}
            </div>
        </div>
